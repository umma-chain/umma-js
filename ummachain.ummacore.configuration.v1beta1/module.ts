...
import { MsgUpdateFees } from "./types/configuration/v1beta1/msgs";
import { MsgUpdateConfig } from "./types/configuration/v1beta1/msgs";

import { Config as typeConfig} from "./types"
import { Fees as typeFees} from "./types"
  ...
  return {
		...
		async sendMsgUpdateFees({ value, fee, memo }: sendMsgUpdateFeesParams): Promise<DeliverTxResponse> {
			...
			try {			
				...
				let msg = this.msgUpdateFees({ value: MsgUpdateFees.fromPartial(value) })
		...
		async sendMsgUpdateConfig({ value, fee, memo }: sendMsgUpdateConfigParams): Promise<DeliverTxResponse> {
			...
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgUpdateConfig({ value: MsgUpdateConfig.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
