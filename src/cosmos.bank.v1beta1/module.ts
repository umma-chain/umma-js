...
import {
  SendAuthorization as typeSendAuthorization, Params as typeParams, SendEnabled as typeSendEnabled, Input as typeInput, Output as typeOutput, Supply as typeSupply, DenomUnit as typeDenomUnit, Metadata as typeMetadata, Balance as typeBalance,
} from "./types";
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => ({
  ...
  async sendMsgMultiSend({ value, fee, memo }: sendMsgMultiSendParams): Promise<DeliverTxResponse> {
    ...
    try {
      ...
      const msg = this.msgMultiSend({ value: MsgMultiSend.fromPartial(value) });
  ...
  async sendMsgSend({ value, fee, memo }: sendMsgSendParams): Promise<DeliverTxResponse> {
    ...
    try {
      const { address } = (await signer.getAccounts())[0];
      const signingClient = await SigningStargateClient.connectWithSigner(addr, signer, { registry, prefix });
      const msg = this.msgSend({ value: MsgSend.fromPartial(value) });
      return await signingClient.signAndBroadcast(address, [msg], fee || defaultFee, memo);
    } catch (e: any) {
