...
export class IgniteClient extends EventEmitter {
  ...
  async signAndBroadcast(msgs: EncodeObject[], fee: StdFee, memo: string) {
    if (this.signer) {
      ...
      return signingClient.signAndBroadcast(address, msgs, fee || defaultFee, memo);
    }
  ...
  async useKeplr(keplrChainInfo: Partial<ChainInfo> = {}) {
    ...
    try {
      const { queryClient } = await import("./cosmos.base.tendermint.v1beta1/module");
      ...
      const addrPrefix = this.env.prefix ?? "umma";
